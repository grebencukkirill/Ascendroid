...
public class LevelEditor : MonoBehaviour
{
    ...
    {
        yield return null; // дождатьс€ конца кадра
    ...
    void ExitEditorMode()
    {
        ...
        // ѕосле выхода из режима редактировани€ активируем лазеры
        foreach (LaserController laser in FindObjectsOfType<LaserController>())
        {
    ...
    void AdjustSilhouetteRotation()
    {
        ...
        if (selectedDevicePrefab.name == "LiftPad" || selectedDevicePrefab.name == "GravFlip" || selectedDevicePrefab.name == "DashPad")
        {
            ...
            {
                deviceSilhouette.transform.localScale = new Vector3(1, 1, 1); // Ќормальное положение
            }
            else if (!hasGroundBelow && hasGroundAbove)
            {
                deviceSilhouette.transform.localScale = new Vector3(1, -1, 1); // ѕеревернутое положение
            }
            else if (hasGroundBelow && hasGroundAbove)
            {
    ...
    void CreateEraserSilhouette()
    {
        ...
        sr.color = new Color(1f, 0f, 0f, 0.5f); // ѕолупрозрачный красный
